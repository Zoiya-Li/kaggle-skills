...
- only then broaden to broader web discovery via `WebSearch`
...
- Prefer `IEEE` and `ScienceDirect` first, `ACM` second, and only then broader web discovery via `WebSearch` unless the user asks otherwise.
