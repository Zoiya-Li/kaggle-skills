...
- **当前默认搜索入口**：优先使用 Claude 内建 `WebSearch` 发现候选来源，再用 `WebFetch`、curl 或 CDP 做定向核实。

| 场景 | 工具 |
|------|------|
| 搜索摘要或关键词结果，发现信息来源 | **`WebSearch`** |
| URL 已知，需要从页面定向提取特定信息 | **`WebFetch`**（拉取网页内容，由小模型根据 prompt 提取，返回处理后结果）；必要时再用 **curl** 获取原始 HTML |
...
- 子 Agent 有自主判断能力。主 Agent 的职责是说清楚**要什么**，仅在必要与确信时限定**怎么做**。过度指定步骤会剥夺子 Agent 的判断空间，反而引入主 Agent 的假设错误。**避免 prompt 用词对子 Agent 行为的暗示**：比如「搜索xx」会把子 Agent 锚定到单一搜索入口，而实际上有些反爬站点需要 CDP 直接访问主站才能有效获取内容。主 Agent 写 prompt 时应描述目标（「获取」「调研」「了解」），避免用暗示具体手段的动词（「搜索」「抓取」「爬取」）。如果明确需要搜索发现来源，优先走 `WebSearch`。
