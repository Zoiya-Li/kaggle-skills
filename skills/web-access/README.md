...
Claude Code 原本有 WebSearch、WebFetch，这个 skill 补上的是：**联网策略 + CDP 浏览器操作 + 站点经验积累**。
...
1. 先用 `WebSearch` 找来源
2. 再用 `WebFetch`、curl 或 CDP 读取原始页面
3. 需要登录态、动态渲染或站内导航时切到 CDP
...
- "先用 WebSearch 搜索 xxx 最新进展，再打开最相关的两个链接核实"
- "读一下这个页面：[URL]"
- "去小红书找 xxx 的账号"
- "帮我在创作者平台发一篇图文"
